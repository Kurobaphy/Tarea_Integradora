...
public class UsuarioDao implements Dao<Usuario, Long> {
    ...
    private static final String SELECT_BY_CORREO_O_USUARIO =
            "SELECT * FROM usuarios WHERE correo_electronico = ? OR nombre_usuario = ?";
    private static final String SELECT_BY_CODIGO =
            "SELECT * FROM usuarios WHERE codigo_recuperacion = ?";
    private static final String GUARDAR_CODIGO_RECUPERACION =
            "UPDATE usuarios SET codigo_recuperacion = ?, codigo_expiracion = ? WHERE id = ?";
    private static final String ACTUALIZAR_CONTRASENA_Y_LIMPIAR_CODIGO =
            "UPDATE usuarios SET contrasena_hash = ?, codigo_recuperacion = NULL, codigo_expiracion = NULL WHERE id = ?";
    ...
    public Usuario getByCorreoOUsuario(String valor) {
        try (Connection con = SQLConnector.getConnection();
             PreparedStatement ps = con.prepareStatement(SELECT_BY_CORREO_O_USUARIO)) {
    ...
    public boolean guardarCodigoRecuperacion(Long id, String codigo, LocalDateTime expiracion) {
        try (Connection con = SQLConnector.getConnection();
             PreparedStatement ps = con.prepareStatement(GUARDAR_CODIGO_RECUPERACION)) {
    ...
    /**
     * Paso 6: actualiza la contraseña y deja el código de recuperación en
     * NULL, en la misma operación (para que no se pueda reutilizar).
     */
    public boolean actualizarContrasenaYLimpiarCodigo(Long id, String nuevoHash) {
        try (Connection con = SQLConnector.getConnection();
             PreparedStatement ps = con.prepareStatement(ACTUALIZAR_CONTRASENA_Y_LIMPIAR_CODIGO)) {
    ...
    private Usuario mapRow(ResultSet rs) throws SQLException {
        ...
        Timestamp codigoExp = rs.getTimestamp("codigo_expiracion");
        if (codigoExp != null) {
            u.setCodigoExpiracion(codigoExp.toLocalDateTime());
        }
