...
/**
 * Requerimientos 2.1 y 2.2: ver, crear y editar artículos junto con su ficha
 * técnica. Ver el detalle es público (incluye Invitado); crear/editar exige
 * rol EDITOR o ADMINISTRADOR.
 *
 * NOTA: hasta que exista un RoleFilter centralizado, la validación de rol se
 * hace aquí mismo al inicio de cada acción protegida (esEditorOAdmin()).
 */
@WebServlet("/articulo")
public class ArticuloServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        ficha.setDesarrollador(request.getParameter("desarrollador"));
        ficha.setGenero(request.getParameter("genero"));
        ...
        boolean guardado;
        if (original == null) {
            // Creación: el autor es quien está guardando
            articulo.setAutor(usuario);
            guardado = articuloDao.create(articulo);
        } else {
            // Edición: conserva id, autor ORIGINAL y el id de la ficha técnica existente;
            // "usuario" (quien edita ahora) se pasa aparte para el historial (2.3).
    ...
    /**
     * Eliminar un artículo es más destructivo que editarlo: el ON DELETE
     * CASCADE del esquema se lleva también todo su historial de versiones
     * (Requerimiento 2.3), sin forma de recuperarlo después. Por eso, a
     * diferencia de crear/editar (EDITOR o ADMINISTRADOR), esto se
     * restringe solo a ADMINISTRADOR — RoleFilterArticulo no distingue
     * este caso porque permite EDITOR también, así que el chequeo extra
     * se hace aquí mismo, igual que la reversión en HistorialServlet.
     */
